...
import { Search, Star, Plus, Edit, Trash2, ExternalLink, FolderOpen, ChevronDown, Moon, Sun, Play, Pause, Clock, RotateCcw, Timer } from 'lucide-react'
...
function ProjectCard({ project, onEdit, onDelete, onToggleFavorite, onStatusChange, onToggleTimer, onTimeEdit, onResetStats, onResetTimer }: ProjectCardProps) {
  ...
  return (
    <Card className="h-[280px] bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-xl hover:border-blue-300 dark:hover:border-blue-600 transition-all duration-300 hover:-translate-y-1 rounded-2xl overflow-hidden group relative backdrop-blur-sm cursor-pointer">
      {/* Type and Platform Badges */}
      <div className="absolute top-2 left-2 z-10 flex gap-1">
        <div className="w-8 h-8 rounded-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg flex items-center justify-center border border-gray-200 dark:border-gray-600">
      ...
      >
        ...
        <div className="flex-shrink-0 mb-3">
          <div className="flex items-start justify-between mb-2">
            <h3 className="text-base font-bold line-clamp-2 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors pr-2 min-h-[2.5rem] flex items-center">
              {project.name}
            </h3>
            <Button
              variant="ghost"
              ...
              className="w-6 h-6 p-0 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200 flex-shrink-0 ml-1"
              title={project.isFavorite ? "Retirer des favoris" : "Ajouter aux favoris"}
            >
              <Star
                className={`h-3 w-3 transition-all duration-200 ${project.isFavorite
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          {/* Tools - Zone flexible avec hauteur limitée */}
          <div className="flex-1 mb-3 min-h-[3rem] max-h-[4rem] overflow-hidden">
            {project.tools && (
              <div className="flex flex-wrap gap-1">
                {project.tools.split(',').slice(0, 4).map((tool, index) => (
                  <Badge key={index} variant="secondary" className="text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800 px-2 py-1 rounded-full">
                    {tool.trim()}
                  </Badge>
                ))}
                {project.tools.split(',').length > 4 && (
        ...
        <div className="flex-shrink-0 mt-auto">
          ...
          <div className="flex items-center gap-2 mb-2">
            <Button
              ...
              className={`h-7 px-3 rounded-full text-xs font-medium transition-all duration-200 flex items-center gap-1 flex-1 justify-center ${isTimerActive
            ...
            >
              ...
              <span
                className="font-mono font-bold cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                onDoubleClick={(e) => {
                  e.stopPropagation()
                  onTimeEdit(project)
                }}
            ...
            <Button
              ...
              className={`h-7 px-3 rounded-full text-xs font-medium transition-all duration-200 ${getStatusColor(project.status)} text-white hover:opacity-90 shadow-lg hover:scale-105 cursor-pointer flex-shrink-0`}
          ...
          <div className="flex gap-1 justify-center" onClick={(e) => e.stopPropagation()}>
            {project.resultLink && (
              <Button variant="ghost" size="sm" asChild className="w-6 h-6 p-0 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 hover:scale-110 transition-all duration-200">
            ...
            <Button
              ...
              className="w-6 h-6 p-0 rounded-full text-orange-600 hover:text-orange-700 hover:bg-orange-50 dark:text-orange-400 dark:hover:bg-orange-900/20 hover:scale-110 transition-all duration-200"
            ...
            <Button
              ...
              className="w-6 h-6 p-0 rounded-full text-purple-600 hover:text-purple-700 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20 hover:scale-110 transition-all duration-200"
            ...
            <Button
              ...
              className="w-6 h-6 p-0 rounded-full text-red-600 hover:text-red-700 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20 hover:scale-110 transition-all duration-200"
...
export default function Home() {
  ...
  const handleTimeSave = async () => {
    ...
    try {
      const timeInSeconds = parseTimeInput(newTimeSpent)
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${id}`, {
        ...
        body: JSON.stringify({
          ...
          lastStarted: lastStarted ? lastStarted.toISOString() : null
        })
      })
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-gray-900 dark:to-slate-900">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm">
                  Gérez et organisez tous vos projets informatiques
                </p>
              </div>
            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {predefinedTools.map((tool) => (
                            <Button
                              ...
                              className={`text-xs ${newProject.tools.includes(tool)
                      ...
                      <div className="col-span-3">
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${newProject.type === 'perso'
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${newProject.type === 'pro'
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${newProject.type === 'perso_pro'
                      ...
                      <div className="col-span-3">
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${newProject.platform === 'pc'
                                ? 'bg-green-500 text-white'
                                : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                              }`}
                          >
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${newProject.platform === 'mac'
                                ? 'bg-blue-500 text-white'
                                : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                              }`}
                          >
                            ...
                            {predefinedTools.map((tool) => {
                              ...
                              return (
                                <Button
                                  ...
                                  className={`text-xs ${currentTools.includes(tool)
                                      ? "bg-blue-600 hover:bg-blue-700 text-white"
                                      : "dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-600"
                                    }`}
                                >
                        ...
                        <div className="col-span-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${editingProject?.type === 'perso'
                                  ? 'bg-green-500 text-white'
                                  : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                                }`}
                            >
                            ...
                            <button
                              ...
                              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${editingProject?.type === 'pro'
                                  ? 'bg-blue-500 text-white'
                                  : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                                }`}
                            >
                            ...
                            <button
                              ...
                              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${editingProject?.type === 'perso_pro'
                                  ? 'bg-purple-500 text-white'
                                  : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                                }`}
                            >
                        ...
                        <div className="col-span-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${editingProject?.platform === 'pc'
                                  ? 'bg-green-500 text-white'
                                  : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                                }`}
                            >
                            ...
                            <button
                              ...
                              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${editingProject?.platform === 'mac'
                                  ? 'bg-blue-500 text-white'
                                  : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                                }`}
                            >
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            ...
            <div className="flex items-center space-x-4 text-sm">
              <div className="text-center">
                <div className="text-lg font-semibold text-gray-900 dark:text-white">{projects.length}</div>
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className={`text-xs transition-all duration-200 h-8 ${showFilters
              ...
              <div className="flex flex-wrap gap-2">
                {getSmartSuggestions().map((suggestion, index) => (
                  <Button
                    ...
                    className="text-xs h-7 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:shadow-md transition-all duration-200"
                  >
                    {suggestion.label}
                  ...
                  {getAllTools().slice(0, 8).map(tool => {
                    ...
                    return (
                      <Button
                        ...
                        size="sm"
                        onClick={() => handleToolToggle(tool)}
                        className={`text-xs h-7 transition-all duration-200 ${isSelected
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        {filteredProjects.length === 0 ? (
          ...
          <div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border border-gray-200/50 dark:border-gray-700/50 mb-6">
              ...
              <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-2">🧠 Comprendre les métriques</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-gray-600 dark:text-gray-400">
                  <div>
                    <span className="font-medium text-green-600 dark:text-green-400">🧾 Temps déclarable (pro) :</span>
                    <br />Somme de toutes les sessions des projets professionnels, même en parallèle
                    <br />• Comptabilise chaque projet pro séparément
                    <br />• Inclut les périodes de chevauchement (double-comptage)
                  </div>
                  <div>
                    <span className="font-medium text-blue-600 dark:text-blue-400">👨‍💻 Temps réel global :</span>
                    <br />Durée totale pendant laquelle au moins un projet était actif
                    <br />• Chevauchements fusionnés
                    <br />• Pas de double-comptage
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div className="text-center p-6 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl border-2 border-green-200 dark:border-green-700 shadow-lg">
                  <div className="text-lg font-semibold text-green-800 dark:text-green-200 mb-2">
                    🧾 Temps déclarable (pro)
                  </div>
                  <div className="text-4xl font-bold text-green-600 dark:text-green-400 mb-2">
                    {formatTimeForStats(timeStats.declarableTime)}
                  </div>
                  <div className="text-sm text-green-600 dark:text-green-300">
                    Somme de toutes les sessions des projets professionnels, même en parallèle
                  </div>
                </div>

                <div className="text-center p-6 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl border-2 border-blue-200 dark:border-blue-700 shadow-lg">
                  <div className="text-lg font-semibold text-blue-800 dark:text-blue-200 mb-2">
                    👨‍💻 Temps réel global
                  </div>
                  <div className="text-4xl font-bold text-blue-600 dark:text-blue-400 mb-2">
                    {formatTimeForStats(timeStats.realGlobalTime)}
                  </div>
                  <div className="text-sm text-blue-600 dark:text-blue-300">
                    Durée totale pendant laquelle au moins un projet était actif, chevauchements fusionnés (pas de double-comptage)
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl border border-purple-200 dark:border-purple-700 shadow-md">
                  <div className="text-2xl font-bold text-purple-600 dark:text-purple-400 mb-1">
                    {formatTimeForStats(timeStats.totalTimeSpent)}
                  </div>
                  <div className="text-xs text-purple-700 dark:text-purple-300 font-medium">Temps total</div>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-xl border border-orange-200 dark:border-orange-700 shadow-md">
                  <div className="text-2xl font-bold text-orange-600 dark:text-orange-400 mb-1">
                    {formatTimeForStats(timeStats.timeToday)}
                  </div>
                  <div className="text-xs text-orange-700 dark:text-orange-300 font-medium">Moyenne/jour</div>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-cyan-50 to-cyan-100 dark:from-cyan-900/20 dark:to-cyan-800/20 rounded-xl border border-cyan-200 dark:border-cyan-700 shadow-md">
                  <div className="text-2xl font-bold text-cyan-600 dark:text-cyan-400 mb-1">
                    {formatTimeForStats(timeStats.timeThisWeek)}
                  </div>
                  <div className="text-xs text-cyan-700 dark:text-cyan-300 font-medium">Cette semaine</div>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/20 rounded-xl border border-pink-200 dark:border-pink-700 shadow-md">
                  <div className="text-2xl font-bold text-pink-600 dark:text-pink-400 mb-1">
                    {formatTimeForStats(timeStats.timeThisMonth)}
                  </div>
                  <div className="text-xs text-pink-700 dark:text-pink-300 font-medium">Ce mois</div>
                </div>
              </div>

              {/* Projet en cours */}
              {projects.some(p => p.isRunning) && (
                <div className="mt-4 p-3 bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 rounded-xl border border-orange-200 dark:border-orange-800 shadow-md">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></div>
                    <span className="text-sm font-medium text-orange-800 dark:text-orange-200">
                      {projects.filter(p => p.isRunning).length} projet(s) en cours
                    </span>
                  </div>
                </div>
              )}
      ...
      <Dialog open={isTimeEditModalOpen} onOpenChange={setIsTimeEditModalOpen}>
        <DialogContent className="sm:max-w-[400px] dark:bg-gray-800 dark:border-gray-700">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="timeSpent" className="text-right dark:text-gray-300">
                Temps
              </Label>
              <Input
                id="timeSpent"
                ...
                placeholder="HH:MM:SS ou MM:SS ou SS"
                className="col-span-3 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
              />
            </div>
            <div className="text-xs text-gray-500 dark:text-gray-400 text-center">
              Formats acceptés : 1:30:00 (1h30), 30:00 (30min), 45 (45sec)
            </div>
          </div>
          <DialogFooter>
            <Button
